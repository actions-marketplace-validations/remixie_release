...
(async () => {
  try {
    ...
    cliPath = `node_modules/${cli}/${
      JSON.parse(
        (await fs.readFile(`node_modules/${cli}/package.json`)).toString()
      ).bin[cli]
    }`;
    ...
    core.info(
      `Creating release on GitHub${
        publishToGithub ? " and publishing to GitHub registry" : ""
      }...`
    );
...
async function semanticRelease(
  ...
  if (release) {
    await exec.exec(
      "node",
      [path, "--no-ci", "--extends", `${__dirname}/../release.config.js`],
      { env }
    );
  }
